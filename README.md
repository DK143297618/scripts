...
| [realm-manager.sh](realm-manager.sh) | Realm port-forward manager — rules carry **remarks**; prefers the glibc build, auto-falls back to glibc2.28 → musl if it can't start; binaries via `docker.mmzs.space`, sha256 verified against the GitHub digest | `curl -fsSL https://raw.githubusercontent.com/DK143297618/scripts/main/realm-manager.sh -o realm-manager.sh && chmod +x realm-manager.sh && ./realm-manager.sh` |
